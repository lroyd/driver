...
//#define SABRESD_ALS_INT		IMX_GPIO_NR(6, 15)
...
#define SABRESD_CODEC_PWR_EN	IMX_GPIO_NR(5, 5)//modify by yonhan for audio codec power
...
#define SABRESD_HEADPHONE_DET	IMX_GPIO_NR(3, 20)
...
#define SABRESD_LCD_VCC		IMX_GPIO_NR(1, 10)
#define SABRESD_LCD_BACKLIGHT	IMX_GPIO_NR(5, 17)  // editted  by yonhan at 2018.8.6
/* add by bhj */

#define SABRESD_OPER_OPEN       IMX_GPIO_NR(5, 8)  //button for doors opening

#define SABRESD_RESET_FACTORY      IMX_GPIO_NR(6, 31)  //button for reset factory
#define SABRESD_INPUT_TAMPER      IMX_GPIO_NR(5, 6)  //button for tamper
#define SABRESD_INPUT_INPUT0      IMX_GPIO_NR(4, 6)  //button for input default
...
#define SABRESD_VCNL_POW		IMX_GPIO_NR(3, 29)
#define SABRESD_I2C_INT_VCNL	IMX_GPIO_NR(3, 28)
...
static struct mxc_audio_platform_data wm8962_data = {
	...
	.hp_gpio = SABRESD_HEADPHONE_DET,
	.hp_active_low = 1,
	.mic_gpio = SABRESD_HEADPHONE_DET,
	.mic_active_low = 1,
...
static struct i2c_board_info mxc_i2c1_board_info[] __initdata = {
	{
		I2C_BOARD_INFO("vcnl4200", 0x51),
		.irq = gpio_to_irq(SABRESD_I2C_INT_VCNL),
	},
/*nan	{
...
static struct ipuv3_fb_platform_data sabresd_fb_data[] = {
	{ /*fb0*/
	.disp_dev = "ldb",
	.interface_pix_fmt = IPU_PIX_FMT_RGB24,//add by bhj for LTN101AL03 10.1 diagonal LCD
//#ifdef CONFIG_FOR_7
#if 0
	.mode_str = "LDB-1024X600",//add by bhj
	.default_bpp = 16,//add by bhj
#else
	.mode_str = "LDB-WXGA",//add by bhj for LTN101AL03 10.1 diagonal LCD
	.default_bpp = 32,//add by bhj for LTN101AL03 10.1 diagonal LCD
#endif
	.int_clk = false,
	.late_init = false,
	}, {
...
static void hdmi_disable_ddc_pin(void)
{
	if (cpu_is_mx6dl())
		...
		mxc_iomux_v3_setup_multiple_pads(mx6q_sabresd_i2c2_pads, ARRAY_SIZE(mx6q_sabresd_i2c2_pads));
}
...
static struct led_pwm mx6_sabresd_pwm_leds[] = {
	{
		.name = "pwm2_led_red",
		.pwm_id = 1,
		.max_brightness	= 255,
...
static struct led_pwm_platform_data mx6_sabresd_pwm_data = {
	.num_leds = ARRAY_SIZE(mx6_sabresd_pwm_leds),
	.leds = mx6_sabresd_pwm_leds,
};

static struct platform_device mx6_sabresd_leds_pwm_device = {
	.name = "leds_pwm",
	.id	= -1,
	.dev = {
		.platform_data = &mx6_sabresd_pwm_data,
	},
};

static void __init imx6q_add_device_pwm_leds(void)
{
	platform_device_register(&mx6_sabresd_leds_pwm_device);
}
...
#ifdef CONFIG_TOUCH_WAKEUP
//	GPIO_BUTTON(SABRESD_VOLUME_UP, KEY_VOLUMEUP, 1, "volume-up", 1, 1),
...
//	GPIO_BUTTON(SABRESD_VOLUME_UP, KEY_VOLUMEUP, 1, "volume-up", 0, 1),
	...
	GPIO_BUTTON(SABRESD_OPER_OPEN, KEY_DOOR_OPEN, 1, "door_open", 1, 1),
    GPIO_BUTTON(SABRESD_RESET_FACTORY,KEY_RESET_FACTORY,1,"reset_key",1,1),
    GPIO_BUTTON(SABRESD_INPUT_TAMPER,KEY_INPUT_TAMPER,1,"tamper_key",1,1),
    GPIO_BUTTON(SABRESD_INPUT_INPUT0,KEY_INPUT_INPUT0,1,"input0_key",1,1),
...
/*		if (enet_to_gpio_6) {
	...
	/* rs485 dir */
	gpio_request(SABRESD_RS485_DIR, "rs485_dir");
	gpio_direction_output(SABRESD_RS485_DIR, 0);
...
*/
	...
	imx6q_add_device_pwm_leds();

	
	gpio_request(SABRESD_VCNL_POW, "vcnl4200-power");
	gpio_direction_output(SABRESD_VCNL_POW,0);

	gpio_request(SABRESD_I2C_INT_VCNL, "vcnl4200-int");
	gpio_direction_input(SABRESD_I2C_INT_VCNL);	
...
//		fec_data.gpio_irq = -1;
	...
	*/
	...
	mxc_iomux_v3_setup_multiple_pads(mx6q_sabresd_kpp_pads, ARRAY_SIZE(mx6q_sabresd_kpp_pads));	//KPP
	//platform_device_register(&sabresd_keypad);	//KPP
...
*/
        ...
        gpio_request(SABRESD_CHARGE_DONE, "pwm2_led_red");//add by bhj
        gpio_direction_output(SABRESD_CHARGE_DONE, 0);//add by bhj
        gpio_request(SABRESD_CSI0_RST, "pwm3_led_green");//add by bhj
        gpio_direction_output(SABRESD_CSI0_RST, 0);//add by bhj
...
static void __init mx6q_sabresd_reserve(void)
{
	...
	for (i = 0; i < fb_array_size; i++) {
		if (sabresd_fb_data[i].res_size[0]) {
			if (sabresd_fb_data[i].res_base[0]) {
				memblock_reserve(sabresd_fb_data[i].res_base[0],
						 sabresd_fb_data[i].res_size[0]);
				memblock_remove(sabresd_fb_data[i].res_base[0],
						sabresd_fb_data[i].res_size[0]);
				sabresd_fb_data[i].late_init = true;
#if defined(CONFIG_MX6_CLK_FOR_BOOTUI_TRANS_LCD_IPU1_DI0) || defined(CONFIG_MX6_CLK_FOR_BOOTUI_TRANS_LVDS_IPU1_DI1)
				ipu_data[0].bypass_reset = true;
#endif
#ifdef CONFIG_MX6_CLK_FOR_BOOTUI_TRANS_HDMI_IPU2_DI0
				ipu_data[1].bypass_reset = true;
#endif
			} else {
